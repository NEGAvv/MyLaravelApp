...
export default function Dashboard({ auth, seriesPaginated, sortedSeries }) {
    ...
    const handleSortButtonClick = (newSortOrder) => {
        ...
        axios
            .get("/dashboard", { params: { sort_order: newSortOrder } })
            .then((response) => {
    ...
    const handleSearch = (query) => {
        ...
        if (query.trim() !== "") {
            ...
            const filteredData = sortedSeries.filter(
                (seriesItem) =>
                    seriesItem.name
                        .toLowerCase()
                        .includes(query.toLowerCase()) ||
                    (seriesItem.user &&
                        seriesItem.user.name
                            .toLowerCase()
                            .includes(query.toLowerCase()))
            );
    ...
    if (sortOrder === "asc") {
        sortedSeriesData.sort(
            (a, b) =>
                new Date(a.date_of_creation) - new Date(b.date_of_creation)
        );
                            ...
                            <div className="p-6 py-2 text-gray-900">
                                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between">
                                    <div className="max-w-xl sm:max-w-md mb-4 sm:mb-0">
                                        ...
                                        <div>
                                            Replies{" "}
                                            <p className="text-sm circle">
                                                {seriesItem.comments_count}
                                            </p>
                                        </div>
                                    </div>
                    ...
                    <div className="flex justify-left mb-4">
                        {searchQuery ? null : (
                            <>
                                ...
                                {[
                                    ...Array(seriesPaginated.last_page).keys(),
                                ].map((pageNumber) => (
                                    <button
                                        key={pageNumber + 1}
                                        style={{
